            ...
            <div class="card-body p-0">
                ...
                <div class="p-4 border-bottom d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
                    <div class="d-flex align-items-start gap-3">
                        ...
                        <div>
                            <div class="fw-semibold text-dark fs-6">{{ $user->getFullName() }}</div>
                            <div class="text-muted small">{{ $user->email }}</div>
                            <div class="text-muted small mt-1">
                                <span class="me-2"><i class="bi bi-calendar3 me-1"></i>DOB: {{ $user->dob?->format('d M Y') ?? 'N/A' }}</span>
                                <span class="me-2">&bull; Requested: <span class="badge bg-secondary">{{ $user->getRoleLabel() }}</span></span>
                                <span>&bull; Registered {{ $user->created_at->diffForHumans() }}</span>
                            </div>
                        </div>
                    </div>

                    <form method="POST" action="{{ route('users.approve', $user) }}" class="d-flex align-items-center gap-2 flex-shrink-0">
                        @csrf
